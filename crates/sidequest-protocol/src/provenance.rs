...
//! produced it.
//!
//! They used to live in `sidequest-genre::resolver`; they moved here so
//! that `sidequest-protocol` can depend on them without creating a
//! dependency cycle (the genre crate already depends on protocol).
//! `sidequest-genre::resolver` re-exports the same names for backward
//! compatibility.
