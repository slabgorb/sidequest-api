...
fn dispatch_connect_calls_party_reconciliation() {
    ...
    assert!(
        source.contains("party_reconciliation"),
        "dispatch/connect.rs must import or call party_reconciliation module"
    );

    assert!(
        source.contains("PartyReconciliation::reconcile")
            || source.contains("party_reconciliation::reconcile"),
...
fn dispatch_connect_emits_party_reconciliation_otel_span() {
    ...
    assert!(
        source.contains("session.resume.party_reconciliation"),
    ...
    assert!(
        source.contains("players_moved")
            || source.contains("old_location")
            || source.contains("before_locations"),
...
fn party_reconciliation_module_declared_in_lib() {
    let source = game_lib_source();

    assert!(
        source.contains("pub mod party_reconciliation"),
