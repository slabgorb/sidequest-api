...
pub use dispatch::beat::{
    apply_beat_dispatch, apply_beat_edge_deltas, apply_beat_edge_deltas_resolved,
    BeatDispatchOutcome, EdgeDeltaOutcome,
};
    ...
    match &msg {
        GameMessage::SessionEvent { payload, .. } if payload.event == "connect" => {
            let mut responses = dispatch::connect::dispatch_connect(
                payload,
                &mut dispatch::connect::ConnectContext {
                    session,
                    builder,
                    player_name_store,
                    character_json_store: character_json,
                    character_name_store: character_name,
                    character_hp,
                    character_max_hp,
                    current_location,
                    discovered_regions,
            ...
            if let (Some(genre), Some(world)) = (session.genre_slug(), session.world_slug()) {
                ...
                *shared_session_holder.lock().await = Some(ss.clone());
                ...
                if session.is_playing() {
                    ...
                    //
                    // If dedup fires, the returned old_pid MUST be fed through
                    // reconcile_removed_player — otherwise the turn_barrier,
        ...
        GameMessage::CharacterCreation { payload, .. } => {
            ...
            dispatch::connect::dispatch_character_creation(
                payload,
                &mut dispatch::connect::ChargenDispatchContext {
                    session,
                    builder,
                    player_name_store,
                    character_json_store: character_json,
        ...
        // Dice throw — rolling player submits their client-side physics result
        // after local Rapier settles. Server treats `payload.face` as the
        // authoritative roll result (physics-is-the-roll, story 34-12). The
        // full dispatch logic lives in `dice_dispatch::handle_dice_throw` so
        // it can be driven directly from integration tests.
