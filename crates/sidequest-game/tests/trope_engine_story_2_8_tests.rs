...
#[test]
fn keyword_accelerator_applies_bonus() {
    let defs = vec![test_trope_def()];
    let mut tropes = vec![test_trope_state("rising_threat")];
    tropes[0].set_progression(0.1);
    tropes[0].set_status(TropeStatus::Progressing);

    TropeEngine::apply_keyword_modifiers(&mut tropes, &defs, "I sense danger ahead");

    // accelerator_bonus is 0.05: 0.1 + 0.05 = 0.15
    assert!((tropes[0].progression() - 0.15).abs() < f64::EPSILON);
}

#[test]
fn keyword_accelerator_case_insensitive() {
    let defs = vec![test_trope_def()];
    let mut tropes = vec![test_trope_state("rising_threat")];
    tropes[0].set_progression(0.1);
    tropes[0].set_status(TropeStatus::Progressing);

    TropeEngine::apply_keyword_modifiers(&mut tropes, &defs, "DANGER is near");

    assert!((tropes[0].progression() - 0.15).abs() < f64::EPSILON);
}

#[test]
fn keyword_accelerator_caps_at_one() {
    let defs = vec![test_trope_def()];
    let mut tropes = vec![test_trope_state("rising_threat")];
    tropes[0].set_progression(0.98);
    tropes[0].set_status(TropeStatus::Progressing);

    TropeEngine::apply_keyword_modifiers(&mut tropes, &defs, "danger everywhere");

    assert!((tropes[0].progression() - 1.0).abs() < f64::EPSILON);
}

// ============================================================================
// AC: Keyword deceleration
// ============================================================================

#[test]
fn keyword_decelerator_applies_penalty() {
    let defs = vec![test_trope_def()];
    let mut tropes = vec![test_trope_state("rising_threat")];
    tropes[0].set_progression(0.5);
    tropes[0].set_status(TropeStatus::Progressing);

    TropeEngine::apply_keyword_modifiers(&mut tropes, &defs, "Everything is safe now");

    // decelerator_penalty is 0.03: 0.5 - 0.03 = 0.47
    assert!((tropes[0].progression() - 0.47).abs() < f64::EPSILON);
}

#[test]
fn keyword_decelerator_floors_at_zero() {
    let defs = vec![test_trope_def()];
    let mut tropes = vec![test_trope_state("rising_threat")];
    tropes[0].set_progression(0.01);
    tropes[0].set_status(TropeStatus::Progressing);

    TropeEngine::apply_keyword_modifiers(&mut tropes, &defs, "feeling calm and safe");

    assert!(tropes[0].progression() >= 0.0);
    assert!(tropes[0].progression() <= 0.01);
}

#[test]
fn keyword_no_modifier_when_no_match() {
    let defs = vec![test_trope_def()];
    let mut tropes = vec![test_trope_state("rising_threat")];
    tropes[0].set_progression(0.5);
    tropes[0].set_status(TropeStatus::Progressing);

    TropeEngine::apply_keyword_modifiers(&mut tropes, &defs, "I look at the flowers");

    assert!((tropes[0].progression() - 0.5).abs() < f64::EPSILON);
}

#[test]
fn keyword_resolved_trope_skipped() {
    let defs = vec![test_trope_def()];
    let mut tropes = vec![test_trope_state("rising_threat")];
    tropes[0].set_progression(0.5);
    tropes[0].set_status(TropeStatus::Resolved);

    TropeEngine::apply_keyword_modifiers(&mut tropes, &defs, "danger danger danger");
