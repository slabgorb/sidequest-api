...
use sidequest_game::npc_actions::{available_actions, select_npc_action, NpcAction, ScenarioRole};
...
fn ac_guilty_has_create_alibi_at_moderate_tension() {
    ...
    let has_alibi = actions
        .iter()
        .any(|(a, _)| matches!(a, NpcAction::CreateAlibi { .. }));
    assert!(
        has_alibi,
        "Guilty NPC should have CreateAlibi available at moderate tension"
    );
}
...
fn ac_guilty_has_destroy_evidence_at_high_tension() {
    ...
    let has_destroy = actions
        .iter()
        .any(|(a, _)| matches!(a, NpcAction::DestroyEvidence { .. }));
    assert!(
        has_destroy,
        "Guilty NPC should have DestroyEvidence at tension > 0.6"
    );
}
...
fn ac_guilty_has_flee_at_extreme_tension() {
    ...
    let has_flee = actions
        .iter()
        .any(|(a, _)| matches!(a, NpcAction::Flee { .. }));
...
fn ac_guilty_has_confess_at_extreme_tension() {
    let actions = available_actions(&ScenarioRole::Guilty, &empty_beliefs(), 0.9);
    let has_confess = actions
        .iter()
        .any(|(a, _)| matches!(a, NpcAction::Confess { .. }));
    assert!(
        has_confess,
        "Guilty NPC should have Confess at tension > 0.8"
    );
}
...
fn ac_witness_has_spread_rumor() {
    ...
    let has_rumor = actions
        .iter()
        .any(|(a, _)| matches!(a, NpcAction::SpreadRumor { .. }));
    assert!(
        has_rumor,
        "Witness NPC with strong suspicion should have SpreadRumor"
    );
}
...
fn ac_accomplice_has_create_alibi() {
    ...
    assert!(
        has_alibi,
        "Accomplice should have CreateAlibi (covering for the guilty)"
    );
}
...
fn ac_tension_scaling_alibi_weight_increases_with_tension() {
    ...
    let alibi_weight_low = low
        .iter()
        .find(|(a, _)| matches!(a, NpcAction::CreateAlibi { .. }))
        .map(|(_, w)| *w)
        .unwrap_or(0.0);
    let alibi_weight_high = high
        .iter()
        .find(|(a, _)| matches!(a, NpcAction::CreateAlibi { .. }))
        .map(|(_, w)| *w)
        .unwrap_or(0.0);
    ...
    assert!(
        alibi_weight_high > alibi_weight_low,
...
fn ac_tension_scaling_no_destroy_evidence_below_threshold() {
    ...
    assert!(
        !has_destroy,
        "DestroyEvidence should not be available at tension 0.3 (threshold is 0.6)"
    );
}
...
fn ac_tension_scaling_no_flee_below_threshold() {
    ...
    assert!(
        !has_flee,
        "Flee should not be available at tension 0.5 (threshold is 0.8)"
    );
}
...
fn ac_low_tension_innocent_only_act_normal() {
    ...
    assert!(actions
        .iter()
        .any(|(a, _)| matches!(a, NpcAction::ActNormal)));
    let non_normal: Vec<_> = actions
        .iter()
        .filter(|(a, _)| !matches!(a, NpcAction::ActNormal))
        .collect();
    assert!(
        non_normal.is_empty() || non_normal.iter().all(|(_, w)| *w < 0.1),
...
fn ac_alibi_creates_false_claim_in_belief_state() {
    ...
    assert!(
        matches!(action, NpcAction::CreateAlibi { .. }),
...
fn edge_tension_zero_only_normal_for_guilty() {
    ...
    assert!(
        act_normal_weight >= 0.9,
        "ActNormal weight at tension 0 should be near 1.0, got {}",
        act_normal_weight
    );
}
